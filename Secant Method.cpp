...
public:
	SecantMethod(const string& formula, int decimalPlaces = 3) :expression(formula), decimalPlaces(decimalPlaces) {}
	...
	long double evaluateRoot()
	{
		...
		if (fValues[0] * fValues[1] >= 0)
		{
			printf("Secant Method Is Not Applicable!\n");
			return 0;
		}
	...
	long double evaluateRoot(long double* realValues, long double* fValues, bool enableRoundWithPrecision = true, int currentIteration = 1)
	{
		...
		{
			realValues[0] = realValues[1], realValues[1] = x, fValues[0] = fValues[1], fValues[1] = f;
...
private:
	...
	string insertValuesInExpression(const double& x)
	{
		...
		tempExpression = replace(tempExpression, "cos(2x)", to_string(cos(2*(x))));
...
int main()
{
	long double realValues[2] = {3 , 4};
	long double fValues[2] = {4.76102172 , -5.16400027};
	//long double realValues[2] = { 2 , 3 };
	//long double fValues[2] = { -2.614574483 ,  4.76102172 };
	...
	SecantMethod obj("2 * x * cos(2x) - (x - 2)^2", 5);
	obj.evaluateRoot(realValues, fValues);
}